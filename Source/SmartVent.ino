...
// --- Telegram Bot ---
#define TELE_BOT_TOKEN "YOUR_TELEGRAM_BOT_TOKEN"
...
void callback(char topic[], byte payload[], unsigned int length) {
  ...
  if (antares.getString("batasSuhu") != "null" ) {
    ...
    bool isControlChanged = isStartUp || (TempThreshold != newTempThresh) ||
    (HumidityThreshold != newHumThresh) || (GasThreshold != newGasThresh) || (VentMode != newVentMode);
...
void checkAndSendNotification() {
  ...
  if (kondisiBahaya && !lastNotifCondition) {
    ...
    // String msg2 = "✅ *Kondisi Kembali Normal*";
